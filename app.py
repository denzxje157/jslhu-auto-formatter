...
# Custom CSS cho giao diện lộng lẫy, hiện đại
st.markdown("""
<style>
    .main-header {
        font-size: 2.3rem;
...
# Sidebar hướng dẫn
with st.sidebar:
    st.image("https://lhu.edu.vn/Data/SEO/logo_lhu.jpg", width=200) if os.path.exists("logo.png") else None
    st.title("📌 Hướng dẫn sử dụng")
    st.info("""
    **Bước 1:** Tải tệp bài báo `.docx` của bạn lên hệ thống.
    
    **Bước 2:** Chọn công cụ định dạng mong muốn:
    - 📝 **Vòng Sơ loại Portal** (1 Cột, A4, 11pt, Lề 3-2-3-2cm)
    - 📰 **Bài báo Mẫu BM01** (2 Cột, A4, 10pt, Lề 2.5cm)
    
    **Bước 3:** Nhấn nút **Tiến hành Định dạng** và tải tệp kết quả về máy!
    """)
    st.markdown("---")
...
with col2:
    ...
    if process_btn:
        if uploaded_file is None:
            ...
            with st.spinner("⏳ Hệ thống đang xử lý định dạng bài báo tự động..."):
                ...
                # Thực thi công cụ theo lựa chọn
                ...
                st.success("🎉 Xử lý định dạng thành công 100%!")
    ...
    with st.expander("🔍 Xem chi tiết bảng chấm điểm 24 tiêu chí", expanded=False):
        st.write("""
